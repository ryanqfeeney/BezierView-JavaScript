...
CRV.prototype.init = function() {
	this.freshObject = 1;
	this.min_crv = new vec4();
	this.max_crv = new vec4();
	this.ratio_a = 1;
	this.ratio_b = 0;
};
...
CRV.prototype.krv = function(v00, Deriv, crv_result) {
	...
	crv_result.x = K; // Gaussian curvature
	crv_result.y = H; // Mean curvature
	crv_result.z = Max; // max curvature
	crv_result.w = Min; // min curvature
...
/* v10, v20 and v01, v02 switched compared to desktop version
	this version of bv.js switches u and v directions */
CRV.prototype.crv4 = function(v00, v10, v20, v01, v02, v11, degv, degu, crv_result) {
	//VEC         Deriv[3][3];
...
/*
CRV.prototype.scalebylog = function(h) {
	var newh;
	if (h > 1.0)
		newh = 1 + Math.log(h);
	else if (h < -1.0)
...
CRV.prototype.crv2color = function(in_crv,hi,low) {
	...
	else // gray scale
	{
		...
		color.set( 0.9-0.7*h, 0.9-0.7*h, 0.9-0.7*h );
	}
